...
1. เมื่อ Queue เต็ม การเรียก `xQueueSend` จะเกิดอะไรขึ้น?
    // ถ้าให้ block time > 0 ฟังก์ชันจะ บล็อก รอจนมีที่ว่างหรือจน หมดเวลา; ถ้ายังเต็มอยู่จะคืนค่า errQUEUE_FULL. ถ้า block time = 0 จะ ไม่บล็อก และล้มเหลวทันทีด้วย errQUEUE_FULL
2. เมื่อ Queue ว่าง การเรียก `xQueueReceive` จะเกิดอะไรขึ้น?
    // ถ้า block time > 0 ฟังก์ชันจะ บล็อก รอจนมีข้อความหรือจน หมดเวลา; ถ้ายังว่างอยู่จะคืนค่า errQUEUE_EMPTY. ถ้า block time = 0 จะ ไม่บล็อก และล้มเหลวทันทีด้วย errQUEUE_EMPTY
3. ทำไม LED จึงกะพริบตามการส่งและรับข้อความ?
        // ในโค้ด เรา set LED = 1 → delay สั้น ๆ → set = 0 ภายหลัง xQueueSend/xQueueReceive สำเร็จ จึงเห็นไฟกะพริบเป็น “ตัวชี้วัด” ว่า มีเหตุการณ์ส่ง/รับ เกิดขึ้นจริงตามจังหวะนั้น
