...
|-------|--------|------|---------|-------|-------|---------|
| 1 (ปกติ) |Sensor: 21 | User: 27 | Network: 55 | Timer: 14 | Total: 117 | สังเกต :ทุก Queue ทำงาน ครบ Processor ตอบสนอง ต่อ ทุก เหตุการณ์ LED กะพริบ ครบ ทั้ง 5 ดวง ระบบ เสถียร |
| 2 (ไม่มี Sensor) | Sensor: 0 | User: 5 | Network: 9 | Timer: 2 | Total: 16 | ไม่มีข้อมูล Sensor แต่ระบบยังทำงานกับ User, Network, Timer ได้ปกติ LED Sensor ไม่กะพริบ |
| 3 (Network เร็ว) | 5 | 5 | 38 | 2 | 50 | Network ทำงานถี่มาก Processor ประมวลผลต่อเนื่อง LED Network กะพริบเร็ว CPU load สูงขึ้นแต่ไม่ drop ข้อมูล |

### คำถามสำหรับการทดลอง
1. Processor Task รู้ได้อย่างไรว่าข้อมูลมาจาก Queue ไหน?
    // Processor Task รู้ว่า ข้อมูลมาจาก Queue ไหน โดยใช้ฟังก์ชัน xQueueSelectFromSet() ซึ่งจะส่งคืน handle ของ Queue หรือ Semaphore ที่มีข้อมูลเข้ามาก่อน
2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
    // เมื่อหลาย Queue มีข้อมูลพร้อมกัน ระบบจะเลือก Queue ที่ถูกเพิ่มเข้า Set ก่อน หรือคิวที่พร้อมก่อนในลำดับการตรวจ
3. Queue Sets ช่วยประหยัด CPU อย่างไร?
    // Queue Sets ช่วยประหยัด CPU เพราะ Task รอเหตุการณ์ได้ในจุดเดียว ไม่ต้องวนเช็กหลาย Queue (Polling) ลดการใช้เวลา Idle และ Context Switch
