...
| 3 (Changed Priority) |  22 | 0 | 0|100% |LOW_PRI (priority สูงสุด) เข้าถึงบ่อยและถือ mutex นานสุด; ไม่มี corruption เพราะ Mutex ยังทำงานปกติ |

### คำถามสำหรับการทดลอง
1. เมื่อไม่ใช้ Mutex จะเกิด data corruption หรือไม่?
    // เมื่อไม่ใช้ Mutex ข้อมูลอาจ เกิด data corruption เพราะหลาย task เขียนทรัพยากรพร้อมกัน
2. Priority Inheritance ทำงานอย่างไร?
    // Priority Inheritance คือกลไกที่ ยก priority ของ task ที่ถือ mutex ให้สูงขึ้นชั่วคราว เพื่อป้องกัน priority inversion
3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร?
    // Task priority สูงจะเข้าถึง shared resource ได้ก่อน ส่วน priority ต่ำต้องรอ — แต่ถ้ามี mutex จะควบคุมให้เข้าได้ทีละ task ปลอดภัย
