...
| 2 (Multiple Give) |12 | 7| 1|1 |58.3 % |
| 3 (Short Timeout) |3 |3 | 1|1 |100% |

### คำถามสำหรับการทดลоง
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?
    // เมื่อ give semaphore หลายครั้งติดต่อกัน → ค่าเก็บได้แค่ 1 ครั้ง การ give ซ้ำ ไม่เพิ่มค่า
2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
    // ISR ต้องใช้ xSemaphoreGiveFromISR() เท่านั้น เพราะปลอดภัยและเหมาะกับ context ของ interrupt
3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
    // Binary Semaphore ใช้ ซิงโครไนซ์เหตุการณ์ (ไม่มีข้อมูล) ส่วน Queue ใช้ ส่งข้อมูลแบบ FIFO
