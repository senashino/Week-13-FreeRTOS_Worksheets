...
1. ไฟล์ใดบ้างที่จำเป็นสำหรับโปรเจกต์ ESP-IDF ขั้นต่ำ?
    //ไฟล์จำเป็น: CMakeLists.txt, sdkconfig, และไฟล์โค้ดหลักใน main/
2. ความแตกต่างระหว่าง `hello_esp32.bin` และ `hello_esp32.elf` คืออะไร?
    // .elf: ใช้ดีบัก (มี symbol) .bin: ไฟล์พร้อมแฟลชลงบอร์ด
3. คำสั่ง `idf.py set-target` ทำอะไร?
    // set-target: ตั้งค่าประเภทชิปที่จะ build (เช่น esp32, esp32s3)
4. โฟลเดอร์ `build/` มีไฟล์อะไรบ้าง?
    // build/: เก็บไฟล์คอมไพล์ทั้งหมด เช่น .o, .elf, .bin, .map, และ config ต่าง ๆ
5. การใช้ `vTaskDelay()` แทน `delay()` มีความสำคัญอย่างไร?
    // vTaskDelay(): หน่วงเวลาโดยไม่บล็อก CPU ให้ Task อื่นทำงานต่อได้ ต่างจาก delay() ที่หยุดระบบทั้งหมด
